<script lang="ts">
	...
	onMount(() => {
		// Initialize chart with sample data if none provided
		if (data.length === 0) {
			const now = new Date();
			data = [
...
<div class="chart-card">
	<div class="chart-container relative" bind:this={chartContainer}>
		{#if chartData.length > 0}
			<!-- Line graph -->
			<svg class="absolute inset-0 w-full h-full p-4">
				<!-- Price axis labels -->
				<text x="10" y="15" class="chart-label numeric">{maxPrice}</text>
				{#if maxPrice !== minPrice}
					<text x="10" y="95%" class="chart-label numeric">{minPrice}</text>
				{/if}
		...
		{:else}
			<div class="flex items-center justify-center h-full">
				<p class="text-sm font-mono" style="color: var(--text-secondary);">No price data available</p>
			</div>
		{/if}
	</div>
</div>
